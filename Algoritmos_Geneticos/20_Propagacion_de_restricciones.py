"""
Propagacion de Restricciones (Constraint Propagation)

Es una técnica utilizada en problemas de satisfaccion de restricciones
(CSP) que mejora la eficiencia del proceso de busqueda al eliminar
valores inconsistentes en los dominios de las variables.
...
class CSP:
    ...
    def propagacion_restricciones(self):
        """
        Aplica propagacion de restricciones para eliminar
        valores inconsistentes en los dominios de las variables.
        """
        cambios = True
        while cambios:
            cambios = False
            for (var1, var2), restriccion in self.restricciones.items():
                # Solo propagar si var1 tiene asignacion
                if var1 in self.dominios:
                    for valor in self.dominios[var2][:]:
                        # Si el valor en var2 no es consistente con var1,
                        # eliminarlo del dominio de var2
    ...
    def backtracking(self, asignacion={}):
        ...
        for valor in self.dominios[variable]:
            if self.es_consistente(variable, valor, asignacion):
                ...
                # Realizar propagacion de restricciones
